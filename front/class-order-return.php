		...
		function ced_rnx_return_product_info_callback() {
			...
			if ( $_POST ) {
				...
				if ( isset( $products ) && ! empty( $products ) ) {
					...
					foreach($_POST['products'] as $key_id => $key_value){
						if(!empty($key_value['reason'])){
							$products[ $date ]['to'][$key_id]['subject'] = $key_value['subject'];
							$products[ $date ]['to'][$key_id]['reason'] = $key_value['reason'];
						}
					}
				}
				if ( $pending ) {
				...
				foreach($_POST['products'] as $key_id => $key_value){
					...
					for ( $i = 0;$i < $count;$i++ ) {
						...
						$products[ $date ]['products'][$key_id]['files'][$i] = $fileNameExchange;
					}
				}
						...
						' . do_action( 'wrnx_return_request_before_mail_content', $order_id ) . '
							...
							dl.variation dd {
								...
								<div class="content">
									...
									<div class="reason">
										<h4>' . __( 'Reason of Return', 'woocommerce-refund-and-exchange' ) . '</h4>
										<p>' . $reason . '</p>
									</div>
									<div class="Order">
				...
				if ( isset( $requested_products ) && ! empty( $requested_products ) ) {
					...
					$message .= '<tr>
									<th colspan="2">' . __( 'Return Total', 'woocommerce-refund-and-exchange' ) . ':</th>
				...
				$order = new WC_Order( $order_id );
				$order->update_status( 'wc-warranty-request', 'User Request to Warranty Claim Product' );
		...
		function ced_rnx_order_return_button( $order ) {
			...
			if ( $ced_rnx_return == 'yes' ) {
				...
				if ( isset( $product_datas ) && ! empty( $product_datas ) ) {
					?>
					<h2><?php _e( 'Warranty Claim Requested Product', 'woocommerce-refund-and-exchange' ); ?></h2>
					<?php
					...
					foreach ( $product_datas as $key => $product_data ) {
						...
						?>
						<p><?php _e( 'Following product warranty request made on', 'woocommerce-refund-and-exchange' ); ?> <b><?php echo $date; ?>.</b></p>
						<table class="shop_table order_details">
								...
								foreach ( $return_products as $return_product ) {
									if ( isset( $return_product['item_id'] ) ) {
										if ( $return_product['item_id'] == $item_id ) {
											...
											$pro_price = $return_product['qty'] * $return_product['price'];
										...
										<tr>
											...
											if(isset($return_product['approve_status_key'])){
												?>
												<span style="color:#ff6161;line-height: 14px;"><strong>Warranty Request : </strong><?php 
													if($order_status == 'wc-warranty-rejected'){
														echo "Rejected";
													}else if($order_status == 'wc-exchange-rejected'){
														echo "Rejected";
													}else{
														if($return_product['approve_status_key'] == '1'){
															echo "Pickup for Approved";
														}else if($return_product['approve_status_key'] == '2'){
															echo "Rejected";
														}
													}?>
												</span>
							...
							?>
							<tr>
								<th scope="row" colspan="2"><?php _e( 'Total', 'woocommerce-refund-and-exchange' ); ?></th>
							...
							if ( isset( $added_fees ) ) {
								if ( is_array( $added_fees ) ) {
									...
									?>
									<tr>
								<th scope="row"><?php _e( 'Total Return Amount', 'woocommerce-refund-and-exchange' ); ?></th>
						...
						if ( $product_data['status'] == 'complete' ) {
							...
							?>
							<p><?php _e( 'Above product warranty request is approved on', 'woocommerce-refund-and-exchange' ); ?> <b><?php echo $appdate; ?>.</b></p>
							<?php
						}
						...
						if ( $product_data['status'] == 'cancel' ) {
							...
							$appdate = date_format( $appdate, $format );
							?>
							<p><?php _e( 'Above product warranty request is cancelled on', 'woocommerce-refund-and-exchange' ); ?> <b><?php echo $appdate; ?>.</b></p>
							<?php
						}
					}
											...
											if ( $ced_rnx_next_return ) {
												if ( $ced_rnx_return_button_show ) {
													...
													<form action="<?php echo esc_html( add_query_arg( 'order_id', $order_id, $return_url ) ); ?>" method="post">
														<!-- <input type="hidden" value="<?php echo esc_html( $order_id ); ?>" name="order_id">
														<p><input type="submit" class="btn button" value="<?php _e( 'Return Request', 'woocommerce-refund-and-exchange' ); ?>" name="ced_new_return_request"></p> -->
													</form>
													<?php
												}
											}
